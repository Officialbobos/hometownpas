...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- ADMIN MESSAGE MODAL ELEMENTS (Still defined, but unused in the flow) ---
    // These are kept to avoid errors, but the logic using them is removed.
    ...
    if (initiateTransferBtn) {
        initiateTransferBtn.addEventListener('click', function(e) {
            ...
            // MODIFIED: Skip Admin Message Modal and proceed directly to PIN modal
            initiatePinModalSequence();
        });
    }

    // Step 2 & 3: Admin Modal Accept/Cancel Logic REMOVED/UNUSED
    // The admin message modal logic has been removed from the flow as requested.

    // Step 4: Handle the PIN confirmation form submission (Existing logic remains)
    ...
    // Step 5: Handle PIN modal cancellation (Existing logic remains)
    if (cancelPinBtn) {
    ...
    function validateFormBeforePinModal() {
        ...
        switch (method) {
            ...
            case 'external_canada_eft':
                // Require all Canadian fields *unless* a saved payee is selected and the fields are disabled
                if (!canadaPayeeSelect || !canadaPayeeSelect.value || (canadaRecipientFields[0] && !canadaRecipientFields[0].disabled)) {
        ...
        fieldsToRequire.forEach(id => {
            ...
            // Only set required if the field is not disabled (which happens if a saved payee is selected for Canada)
